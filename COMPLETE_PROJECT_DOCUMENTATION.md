...
The repository includes PlantUML diagrams that show how design patterns map to the code and how the runtime flows operate.

- **Class diagram:** `diagrams/class-diagram.puml` — shows pattern implementations and class relationships.
- **Sequence diagrams:**
  - `diagrams/sequence-transfer.puml` — transfer flow (approval chain, debit/credit, notifications)
  - `diagrams/sequence-interest.puml` — apply interest flow (facade → accounts → strategy → notify)
  - `diagrams/sequence-notification.puml` — notification delivery flow (account → observers)
